---
sidebar_position: 3
sidebar_label: AOI Step-By-Step
---
...
import Identify from '/docs/partials/platform_api/request/_identify.mdx';
import IdentifyResponse from '/docs/partials/platform_api/response/_identify.mdx';
...
import SearchInstitutions from '/docs/partials/platform_api/request/_search_institutions_aoi.mdx';
...
The example searches for MX Bank, which is [MX's institution for testing and development](xxx). It also limits results to institutions that support AOI.
...
Make a POST request to the `/users/{user_guid}/members` endpoint. The response includes the newly created `member`.
  ...
  * In the body, set the `skip_aggregation` parameter to `true`.
    * A standard aggregation is kicked off automatically whenever a member is created unless you explicitly prevent it using `skip_aggregation`. You may or may not want to receive account and transaction data with your owner data, depending on your use case. The example shows `skip_aggregation: true`..
...
Make a POST request to the `/users/{user_guid}/members/{member_guid}/identify` endpoint. This request only starts the AOI process; actually reading the aggregated data comes later. The response will be a `member` object with information about the state of the AOI process that just started.
...
<CheckStatus job_type="AOI" />

## 7. Deal with Multifactor Authentication

Aggregation-type jobs such as AOI may trigger multifactor authentication (MFA) from the institution associated with the member. This is indicated by `connection_status: CHALLENGED`.

Please see the detailed section on [answering MFA challenges](/products/account_owner_identification/dealing_with_mfa) for more information. For this part of the guide, we'll assume there was no MFA and move on to the next step.

## 8. List Account Owners

The AOI job was successful and you can now list the owners for all accounts associated with the member.

Make a `GET` request to the `/users/{user_guid}/members/{member_guid}/account_owners` endpoint.
